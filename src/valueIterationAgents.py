...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for next_state, prob in next_state_and_probs:
            q_value += prob * (self.mdp.getReward(state, None, None) + self.discount * self.values[next_state])
        return q_value
        ...
        for _ in range(self.iterations):
            ...
            for state in self.mdp.getStates():
                if not self.mdp.isTerminal(state):
                    new_values[state] = self.getQValue(state, self.getAction(state)) # tiramos o cálculo da reward daqui e colocamos no cálculo 
                                                                                     # do q-value para passar nos testes do autograder :/
